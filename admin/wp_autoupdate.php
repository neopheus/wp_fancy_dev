...
class WP_AutoUpdate
{
	...
	public function __construct( $current_version, $update_path, $plugin_slug, $license_user = '', $license_key = '' )
	{
		...
		add_action( 'in_plugin_update_message-'.$plugin_slug, array( &$this, 'plugin_message' ), 10, 2 );
	}

	public function plugin_message( $plugin_data, $r ) {

		$message = false;

		if($this->license_key == false)
			$message = __('Please enter a valid Envato Item Purchase code in order to auto-update!', 'radykal' );
		else if( !isset($r->package) )
			$message = __('The Envato Item Purchase Code is not correct! Please enter a valid one in order to auto-update.', 'radykal' );
	...
	public function check_update( $transient )
	{
		...
		if(isset($remote_version->error) && $remote_version->error == 'http_error')
			return $transient;
